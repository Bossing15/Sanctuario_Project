...
const ProductEditor = ({ product, isOpen, onClose, onSave, canManageServices }) => {
  ...
  useEffect(() => {
    if (product) {
      ...
      setFormData({
        ...
        pricing_title: product.pricing_title || `PURCHASE ${(product.title || 'PRODUCT').toUpperCase()} PACKAGE`,
        pricing_subtitle: product.pricing_subtitle || 'Select your package and proceed to payment.',
        package_title: product.package_title || `${product.title || 'Product'} Package`,
        package_description: product.package_description || `Complete ${(product.title || 'product').toLowerCase()} for your loved one`,
        package_note: product.package_note || 'Price will be calculated at checkout',
      });
      
      // Set preview image - construct the full URL
      if (product.image_path) {
        console.log('Image path found:', product.image_path);
        const imagePath = `/storage/${product.image_path}`;
        console.log('Full image URL:', imagePath);
        setPreviewImage(imagePath);
      } else {
        console.log('No image path in product data');
      ...
      setFormData({
        ...
        status: 'Active',
        image_path: '',
        pricing_title: 'PURCHASE PRODUCT PACKAGE',
        pricing_subtitle: 'Select your package and proceed to payment.',
        package_title: 'Product Package',
        package_description: 'Complete product for your loved one',
